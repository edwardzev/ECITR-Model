...
If the draft has no explicit bounded failure boundary, `complete` attempts a deterministic boundary-recovery lane first. That lane writes a staged recovery packet only when it can recover cited unresolved boundaries from linked evidence payloads. If no honest recovery is possible, completion fails; the batch runner rejects the unusable draft instead of parking it.
...
Run the next live draft batch through bounded completion plus the current approval gate. The batch auto-amends drafts when bounded completion can make them approval-ready, and rejects drafts that still cannot satisfy the gate:

```bash
npm run batch:cases -- --limit 30
```

By default, the batch runner skips case ids that hit prior processing failures in `batch-*-results.json` logs under `.local/review-drafts`, so repeated batches advance into untouched drafts instead of reprocessing the same failing queue head.

Include previously failed drafts again only if you are explicitly re-testing them after an upstream enrichment change:

```bash
npm run batch:cases -- --limit 30 --include-previously-failed
```
